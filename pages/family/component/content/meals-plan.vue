...
<template>
  ...
  <!-- 饮食打卡弹窗，治手账式设计 -->
  <view class="bottom-modal-mask" v-if="showCheckinPopup" @click="closeCheckinPopup">
    <view class="meal-checkin-modal" @click.stop :style="themeStyle">
      <view class="mc-head">
        <text class="mc-title">📸 美食打卡 · {{ checkinMeal?.name }}</text>
        <view class="mc-close" @click="closeCheckinPopup">✕</view>
      </view>
      ...
      <scroll-view scroll-y class="mc-scroll-body" :show-scrollbar="false">
        ...
        <!-- 3. 美食随手拍 -->
        <view class="mc-group">
          <view class="mc-label">📸 美食照片 (复用现存上传接口)</view>
    ...
    success: async (res) => {
      ...
      try {
        ...
        if (response && response.data && response.data.coverUrl) {
          checkinData.value.photo = response.data.coverUrl
  ...
  try {
    ...
    if (props.familyCode) {
      try {
        ...
        // 云端保存整天三餐，带上 record 字段扩展
        const formattedMeals = meals.value.map(meal => {
          const isCurrent = meal.name === m.name
          return {
            meal_name: meal.name,
            done: meal.done ? 1 : 0,
            recipes: meal.recipes || [],
            record: isCurrent ? record : (meal.record || null)
          }
        })
        await familyApi.saveDailyMeals(props.familyCode, today, formattedMeals)
      } catch (err) {
    ...
    uni.setStorageSync(`meal_record_cache_${today}_${m.name}`, record)
    
    // 3. 更新 meals 对象并持久化本地
    m.record = record
    saveMeals()
    
    // 4. 如果是非自己做，且勾选了自动同步消费记录，同步添加记账
    if (checkinData.value.source !== '自己做' && checkinData.value.cost && checkinData.value.syncCost) {
      try {
        const consumptionRecordJson = {
          name: `${m.name}就餐: ${m.recipes.join(' · ') || '便餐'}`,
